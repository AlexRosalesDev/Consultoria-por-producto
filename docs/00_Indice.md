# Índice de Documentación - Proyecto GeoBolivia

## Estructura del Documento de Requerimientos

### 1. Introducción y Contexto
- **01_Introduccion_y_Contexto.md**
  - Introducción y Objetivo del Documento
  - Definición de Requerimiento
  - Ficha del Proyecto
  - Problema a Resolver
  - Objetivo Principal

### 2. Stakeholders y Reglas de Negocio
- **02_Stakeholders.md**
  - Fase A: Definición del Contexto
  - Matriz de Stakeholders
  - Reglas de Negocio y Restricciones

### 3. Requerimientos Funcionales - Módulo Principal
- **03_RF_Modulo_Principal.md**
  - Clasificación de Requerimientos
  - Requerimientos de Negocio
  - Requerimientos de Usuario
  - Tabla de Requerimientos Funcionales (Prioridad MUST)

### 4. Requerimientos Funcionales - Reportes y Análisis
- **04_RF_Reportes.md**
  - Requerimientos Funcionales de Reportes (Prioridad SHOULD/COULD)
  - Requerimientos de Visualización de Datos
  - Criterios de Aceptación para Reportes

### 5. Requerimientos No Funcionales y Cierre
- **05_RNF_y_Restricciones.md**
  - Fase B: Técnicas de Elicitación
  - Requerimientos No Funcionales (URPS+)
  - Validación y Cierre
  - Checklist de Aprobación

---

**Última actualización:** 23/02/2026
**Versión del Documento:** 1.0